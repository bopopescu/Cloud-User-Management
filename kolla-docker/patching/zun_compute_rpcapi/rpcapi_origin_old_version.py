...
class API(rpc_service.API):
    ...
    def carrier_create(self, context, host, container, limits,
                         requested_networks):
        LOG.debug('rpcapi carrier_create host=%s, container=%s XXXXXXXXXX.', host, container)
        self._cast(host, 'carrier_create', limits=limits,
                   requested_networks=requested_networks, container=container)

    @check_container_host
    def carrier_delete(self, context, container, force):
        LOG.debug('rpcapi carrier_delete force=%s, container=%s XXXXXXXXXX.', force, container)
        return self._call('infra1-horizon-container-597d0994', 'carrier_delete',
                          container=container, force=force)

    def container_create(self, context, host, container, limits,
                         requested_networks):
        LOG.debug('rpcapi container_create host=%s, container=%s XXXXXXXXXX.', host, container)
    ...
    def container_delete(self, context, container, force):
        LOG.debug('rpcapi container_delete container=%s XXXXXXXXXX.', container)
        return self._call('infra1-horizon-container-597d0994', 'container_delete',
                          container=container, force=force)
...
#                          container=container, force=force)
    ...
    def carrier_show(self, context, container):
        return self._call("", 'carrier_show',
                          container=container)
    ...
    def carrier_update(self, context, container, patch):
        LOG.debug('carrier_update=%s, container=%s', patch, container.as_dict())
        return self._call("", 'carrier_update',
                          container=container, patch=patch)
