...
class Connection(object):
    ...
    def _do_update_providerregion(self, providerregion_id, values):
        ...
        with session.begin():
            query = model_query(models.Providerregion, session=session)
            query = add_identity_filter(query, providerregion_id)
            LOG.debug('_do_update_providerregion xxxxxx providerregion_id =%s, query=%s, values=%s', providerregion_id, query, values)
            try:
                ref = query.with_lockmode('update').one()
            except NoResultFound:
    ...
    def _do_update_user(self, user_id, values):
        ...
        with session.begin():
            query = model_query(models.User, session=session)
            query = add_identity_filter(query, user_id)
            LOG.debug('_do_update_user xxxxxx user_id =%s, query=%s, values=%s', user_id, query, values)
            try:
                ref = query.with_lockmode('update').one()
            except NoResultFound:
    ...
    # carrier section
    def _add_carriers_filters(self, query, filters):
        if filters is None:
            filters = {}
    ...
    def list_carriers(self, context, filters=None, limit=None,
                        marker=None, sort_key=None, sort_dir=None):
        query = model_query(models.Carrier)
        #query = self._add_tenant_filters(context, query)
        query = self._add_carriers_filters(query, filters)
        LOG.debug('sqlalchemy api.py list_carriers xxx query=%s, xxx',(query))
        LOG.debug('The sort_key of carriers is %s', (sort_key))
        return _paginate_query(models.Carrier, limit, marker,
                               sort_key, sort_dir, query)

    def _validate_unique_carrier_name(self, context, name):
        lowername = name.lower()
        base_query = model_query(models.Carrier).\
            filter(func.lower(models.Carrier.name) == lowername)
        carrier_with_same_name = base_query.count()
        if carrier_with_same_name > 0:
            raise exception.CarrierAlreadyExists(field='name',
                                                   value=lowername)

    def create_carrier(self, context, values):
        # ensure defaults are present for new carriers
        LOG.debug('sqlalchemy api.py create_carrier xxx values=%s' % (values))
        if not values.get('uuid'):
            values['uuid'] = uuidutils.generate_uuid()

        if values.get('name'):
            self._validate_unique_carrier_name(context, values['name'])
    ...
    def get_carrier_by_uuid(self, context, carrier_uuid):
        query = model_query(models.Carrier)
        #query = self._add_tenant_filters(context, query)
        query = query.filter_by(uuid=carrier_uuid)
        try:
            return query.one()
        except NoResultFound:
            raise exception.CarrierNotFound(carrier=carrier_uuid)

    def get_carrier_by_name(self, context, carrier_name):
        query = model_query(models.Carrier)
        query = self._add_tenant_filters(context, query)
        query = query.filter_by(name=carrier_name)
        try:
            return query.one()
        except NoResultFound:
            raise exception.CarrierNotFound(carrier=carrier_name)
        except MultipleResultsFound:
            raise exception.Conflict('Multiple carriers exist with same '
                                     'name. Please use the carrier uuid '
                                     'instead.')

    def destroy_carrier(self, context, carrier_id):
        session = get_session()
        with session.begin():
            query = model_query(models.Carrier, session=session)
            query = add_identity_filter(query, carrier_id)
            count = query.delete()
            if count != 1:
                raise exception.CarrierNotFound(carrier_id)

    def update_carrier(self, context, carrier_id, values):
        # NOTE(dtantsur): this can lead to very strange errors
        if 'uuid' in values:
            msg = _("Cannot overwrite UUID for an existing Carrier.")
            raise exception.InvalidParameterValue(err=msg)

        if 'name' in values:
            self._validate_unique_carrier_name(context, values['name'])

        return self._do_update_carrier(carrier_id, values)

    def _do_update_carrier(self, carrier_id, values):
        session = get_session()
        with session.begin():
            query = model_query(models.Carrier, session=session)
            query = add_identity_filter(query, carrier_id)
            LOG.debug('_do_update_carrier xxxxxx carrier_id =%s, query=%s, values=%s', carrier_id, query, values)
            try:
                ref = query.with_lockmode('update').one()
            except NoResultFound:
                raise exception.CarrierNotFound(carrier=carrier_id)
