...
INSTANCETYPE_CREATE_ATTRS = zun_client.instancetypes.CREATION_ATTRIBUTES
...
def _cleanup_instancetype_params(attrs, check, **params):
    args = {}
    run = False
...
def instancetype_create(request, **kwargs):
    LOG.debug('xxx start instancetype_create xxx %s' % kwargs)
    args, run = _cleanup_params(INSTANCETYPE_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).instancetypes.create(**args)
    return response
...
def instancetype_update(request, id, **kwargs):
    '''Update Container

    Get current Instancetype attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).instancetypes.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_instancetype_params(INSTANCETYPE_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client instancetype_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).instancetypes.update(id, **args)

    # do update
    if name:
        zunclient(request).instancetypes.rename(id, name)
        args["id"] = name
...
def instancetype_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).instancetypes.delete(id, force=force)
...
def instancetype_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).instancetypes.list(limit, marker, sort_key,
                                              sort_dir)
...
def instancetype_show(request, id):
    return zunclient(request).instancetypes.get(id)
