...
@urls.register
class Providervm(generic.View):
    """API for retrieving a single container"""
    url_regex = r'zun/providervms/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Providervm xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.providervm_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        """
        return client.providervm_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        """
        args = client.providervm_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Providervm xxxxxx args=%s, xxx' % (args))
        return args



@urls.register
class Providervms(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/providervms/$'
    ...
    def get(self, request):
        ...
        """
        result = client.providervm_list(request)
    ...
    def delete(self, request):
        ...
        for id in request.DATA:
            client.providervm_delete(request, id)
    ...
    def post(self, request):
        ...
        """
        LOG.debug('providervms restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.providervm_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/providervm/%s' % new_container.uuid,
