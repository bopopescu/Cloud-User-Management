...
STATEMENT_CREATE_ATTRS = zun_client.statements.CREATION_ATTRIBUTES
...
def _cleanup_statement_params(attrs, check, **params):
    args = {}
    run = False
...
def statement_create(request, **kwargs):
    LOG.debug('xxx start statement_create xxx %s' % kwargs)
    args, run = _cleanup_params(STATEMENT_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).statements.create(**args)
    return response
...
def statement_update(request, id, **kwargs):
    '''Update Container

    Get current Statement attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).statements.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_statement_params(STATEMENT_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client statement_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).statements.update(id, **args)

    # do update
    if name:
        zunclient(request).statements.rename(id, name)
        args["id"] = name
...
def statement_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).statements.delete(id, force=force)
...
def statement_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).statements.list(limit, marker, sort_key,
                                              sort_dir)
...
def statement_show(request, id):
    return zunclient(request).statements.get(id)
