...
class Usage(generic.View):
    ...
    url_regex = r'zun/usages/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Usage xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.usage_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        """
        return client.usage_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        """
        args = client.usage_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Usage xxxxxx args=%s, xxx' % (args))
        return args



@urls.register
class Usages(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/usages/$'
    ...
    def get(self, request):
        ...
        """
        result = client.usage_list(request)
    ...
    def delete(self, request):
        ...
        for id in request.DATA:
            client.usage_delete(request, id)
    ...
    def post(self, request):
        ...
        """
        LOG.debug('usages restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.usage_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/usage/%s' % new_container.uuid,
