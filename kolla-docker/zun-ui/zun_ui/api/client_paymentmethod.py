...
PAYMENTMETHOD_CREATE_ATTRS = zun_client.paymentmethods.CREATION_ATTRIBUTES
...
def _cleanup_paymentmethod_params(attrs, check, **params):
    args = {}
    run = False
...
def paymentmethod_create(request, **kwargs):
    LOG.debug('xxx start paymentmethod_create xxx %s' % kwargs)
    args, run = _cleanup_params(PAYMENTMETHOD_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).paymentmethods.create(**args)
    return response
...
def paymentmethod_update(request, id, **kwargs):
    '''Update Container

    Get current Paymentmethod attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).paymentmethods.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_paymentmethod_params(PAYMENTMETHOD_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client paymentmethod_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).paymentmethods.update(id, **args)

    # do update
    if name:
        zunclient(request).paymentmethods.rename(id, name)
        args["id"] = name
...
def paymentmethod_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).paymentmethods.delete(id, force=force)
...
def paymentmethod_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).paymentmethods.list(limit, marker, sort_key,
                                              sort_dir)
...
def paymentmethod_show(request, id):
    return zunclient(request).paymentmethods.get(id)
