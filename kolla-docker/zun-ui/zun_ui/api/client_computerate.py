...
COMPUTERATE_CREATE_ATTRS = zun_client.computerates.CREATION_ATTRIBUTES
...
def _cleanup_computerate_params(attrs, check, **params):
    args = {}
    run = False
...
def computerate_create(request, **kwargs):
    LOG.debug('xxx start computerate_create xxx %s' % kwargs)
    args, run = _cleanup_params(COMPUTERATE_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).computerates.create(**args)
    return response
...
def computerate_update(request, id, **kwargs):
    '''Update Container

    Get current Computerate attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).computerates.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_computerate_params(COMPUTERATE_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client computerate_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).computerates.update(id, **args)

    # do update
    if name:
        zunclient(request).computerates.rename(id, name)
        args["id"] = name
...
def computerate_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).computerates.delete(id, force=force)
...
def computerate_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).computerates.list(limit, marker, sort_key,
                                              sort_dir)
...
def computerate_show(request, id):
    return zunclient(request).computerates.get(id)
