...
@urls.register
class Computerate(generic.View):
    """API for retrieving a single container"""
    url_regex = r'zun/computerates/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Computerate xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.computerate_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        """
        return client.computerate_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        """
        args = client.computerate_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Computerate xxxxxx args=%s, xxx' % (args))
        return args



@urls.register
class Computerates(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/computerates/$'
    ...
    def get(self, request):
        ...
        """
        result = client.computerate_list(request)
    ...
    def delete(self, request):
        ...
        for id in request.DATA:
            client.computerate_delete(request, id)
    ...
    def post(self, request):
        ...
        """
        LOG.debug('computerates restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.computerate_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/computerate/%s' % new_container.uuid,
