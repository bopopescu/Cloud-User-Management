...
@urls.register
class Storagerate(generic.View):
    """API for retrieving a single container"""
    url_regex = r'zun/storagerates/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Storagerate xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.storagerate_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        """
        return client.storagerate_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        """
        args = client.storagerate_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Storagerate xxxxxx args=%s, xxx' % (args))
        return args



@urls.register
class Storagerates(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/storagerates/$'
    ...
    def get(self, request):
        ...
        """
        result = client.storagerate_list(request)
    ...
    def delete(self, request):
        ...
        for id in request.DATA:
            client.storagerate_delete(request, id)
    ...
    def post(self, request):
        ...
        """
        LOG.debug('storagerates restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.storagerate_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/storagerate/%s' % new_container.uuid,
