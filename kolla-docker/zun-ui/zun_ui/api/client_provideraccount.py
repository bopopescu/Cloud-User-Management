...
PROVIDERACCOUNT_CREATE_ATTRS = zun_client.provideraccounts.CREATION_ATTRIBUTES
...
def _cleanup_provideraccount_params(attrs, check, **params):
    args = {}
    run = False
...
def provideraccount_create(request, **kwargs):
    LOG.debug('xxx start provideraccount_create xxx %s' % kwargs)
    args, run = _cleanup_params(PROVIDERACCOUNT_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).provideraccounts.create(**args)
    return response
...
def provideraccount_update(request, id, **kwargs):
    '''Update Container

    Get current Provideraccount attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).provideraccounts.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_provideraccount_params(PROVIDERACCOUNT_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client provideraccount_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).provideraccounts.update(id, **args)

    # do update
    if name:
        zunclient(request).provideraccounts.rename(id, name)
        args["id"] = name
...
def provideraccount_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).provideraccounts.delete(id, force=force)
...
def provideraccount_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).provideraccounts.list(limit, marker, sort_key,
                                              sort_dir)
...
def provideraccount_show(request, id):
    return zunclient(request).provideraccounts.get(id)
