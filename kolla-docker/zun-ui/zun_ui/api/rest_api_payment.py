...
class Payment(generic.View):
    ...
    url_regex = r'zun/payments/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Payment xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.payment_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        """
        return client.payment_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        """
        args = client.payment_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Payment xxxxxx args=%s, xxx' % (args))
        return args



@urls.register
class Payments(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/payments/$'
    ...
    def get(self, request):
        ...
        """
        result = client.payment_list(request)
    ...
    def delete(self, request):
        ...
        for id in request.DATA:
            client.payment_delete(request, id)
    ...
    def post(self, request):
        ...
        """
        LOG.debug('payments restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.payment_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/payment/%s' % new_container.uuid,
