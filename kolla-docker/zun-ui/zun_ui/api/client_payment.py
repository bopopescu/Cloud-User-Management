...
PAYMENT_CREATE_ATTRS = zun_client.payments.CREATION_ATTRIBUTES
...
def _cleanup_payment_params(attrs, check, **params):
    args = {}
    run = False
...
def payment_create(request, **kwargs):
    LOG.debug('xxx start payment_create xxx %s' % kwargs)
    args, run = _cleanup_params(PAYMENT_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).payments.create(**args)
    return response
...
def payment_update(request, id, **kwargs):
    '''Update Container

    Get current Payment attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).payments.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_payment_params(PAYMENT_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client payment_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).payments.update(id, **args)

    # do update
    if name:
        zunclient(request).payments.rename(id, name)
        args["id"] = name
...
def payment_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).payments.delete(id, force=force)
...
def payment_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).payments.list(limit, marker, sort_key,
                                              sort_dir)
...
def payment_show(request, id):
    return zunclient(request).payments.get(id)
