...
PROVIDERVM_CREATE_ATTRS = zun_client.providervms.CREATION_ATTRIBUTES
...
def _cleanup_providervm_params(attrs, check, **params):
    args = {}
    run = False
...
def providervm_create(request, **kwargs):
    LOG.debug('xxx start providervm_create xxx %s' % kwargs)
    args, run = _cleanup_params(PROVIDERVM_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).providervms.create(**args)
    return response
...
def providervm_update(request, id, **kwargs):
    '''Update Container

    Get current Providervm attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).providervms.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_providervm_params(PROVIDERVM_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client providervm_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).providervms.update(id, **args)

    # do update
    if name:
        zunclient(request).providervms.rename(id, name)
        args["id"] = name
...
def providervm_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).providervms.delete(id, force=force)
...
def providervm_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).providervms.list(limit, marker, sort_key,
                                              sort_dir)
...
def providervm_show(request, id):
    return zunclient(request).providervms.get(id)
