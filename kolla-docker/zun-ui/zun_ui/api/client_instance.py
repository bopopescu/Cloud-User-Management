...
INSTANCE_CREATE_ATTRS = zun_client.instances.CREATION_ATTRIBUTES
...
def _cleanup_instance_params(attrs, check, **params):
    args = {}
    run = False
...
def instance_create(request, **kwargs):
    LOG.debug('xxx start instance_create xxx %s' % kwargs)
    args, run = _cleanup_params(INSTANCE_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).instances.create(**args)
    return response
...
def instance_update(request, id, **kwargs):
    '''Update Container

    Get current Instance attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).instances.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_instance_params(INSTANCE_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client instance_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).instances.update(id, **args)

    # do update
    if name:
        zunclient(request).instances.rename(id, name)
        args["id"] = name
...
def instance_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).instances.delete(id, force=force)
...
def instance_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).instances.list(limit, marker, sort_key,
                                              sort_dir)
...
def instance_show(request, id):
    return zunclient(request).instances.get(id)
