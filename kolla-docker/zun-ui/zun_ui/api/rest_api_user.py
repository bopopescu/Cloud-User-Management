...
class Users(generic.View):
    ...
    def get(self, request):
        ...
        LOG.info("result result result result result result result result result result result result result result result result result result result")
        LOG.info(result)
    ...
    #@rest_utils.ajax(data_required=True)
    def create(self, request, userinfo):
        """Create a new Container.

        Returns the new Container object on success.
        If 'run' attribute is set true, do 'run' instead 'create'
        """
        LOG.debug('users restapi POST xxxxxx %s xxxxx %s xxx' % (request, userinfo))
        new_container = client.user_create(request, **userinfo)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/user/%s' % new_container.uuid,
            new_container.to_dict())
