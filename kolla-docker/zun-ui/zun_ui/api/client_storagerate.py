...
STORAGERATE_CREATE_ATTRS = zun_client.storagerates.CREATION_ATTRIBUTES
...
def _cleanup_storagerate_params(attrs, check, **params):
    args = {}
    run = False
...
def storagerate_create(request, **kwargs):
    LOG.debug('xxx start storagerate_create xxx %s' % kwargs)
    args, run = _cleanup_params(STORAGERATE_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).storagerates.create(**args)
    return response
...
def storagerate_update(request, id, **kwargs):
    '''Update Container

    Get current Storagerate attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).storagerates.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_storagerate_params(STORAGERATE_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client storagerate_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).storagerates.update(id, **args)

    # do update
    if name:
        zunclient(request).storagerates.rename(id, name)
        args["id"] = name
...
def storagerate_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).storagerates.delete(id, force=force)
...
def storagerate_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).storagerates.list(limit, marker, sort_key,
                                              sort_dir)
...
def storagerate_show(request, id):
    return zunclient(request).storagerates.get(id)
