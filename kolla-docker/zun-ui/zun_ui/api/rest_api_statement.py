...
@urls.register
class Statement(generic.View):
    """API for retrieving a single container"""
    url_regex = r'zun/statements/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Statement xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.statement_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        """
        return client.statement_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        """
        args = client.statement_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Statement xxxxxx args=%s, xxx' % (args))
        return args



@urls.register
class Statements(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/statements/$'
    ...
    def get(self, request):
        ...
        """
        result = client.statement_list(request)
    ...
    def delete(self, request):
        ...
        for id in request.DATA:
            client.statement_delete(request, id)
    ...
    def post(self, request):
        ...
        """
        LOG.debug('statements restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.statement_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/statement/%s' % new_container.uuid,
