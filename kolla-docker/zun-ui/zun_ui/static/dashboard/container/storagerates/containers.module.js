...
(function() {
  ...
  /**
   * @ngdoc overview
   * @name horizon.dashboard.container.storagerates
   * @ngModule
   * @description
  ...
  angular
    .module('horizon.dashboard.container.storagerates', [
      'ngRoute',
      'horizon.dashboard.container.storagerates.actions',
      'horizon.dashboard.container.storagerates.details'
    ])
    .constant('horizon.dashboard.container.storagerates.events', events())
    .constant('horizon.dashboard.container.storagerates.validStates', validStates())
    .constant('horizon.dashboard.container.storagerates.resourceType', 'OS::Zun::Storagerate')
    .run(run)
    .config(config);

  /**
   * @ngdoc constant
   * @name horizon.dashboard.container.storagerates.events
   * @description A list of events used by Container
   * @returns {Object} Event constants
   */
  function events() {
    return {
      CREATE_SUCCESS: 'horizon.dashboard.container.storagerates.CREATE_SUCCESS',
      DELETE_SUCCESS: 'horizon.dashboard.container.storagerates.DELETE_SUCCESS'
    };
  }
  ...
  run.$inject = [
    ...
    'horizon.dashboard.container.storagerates.basePath',
    'horizon.dashboard.container.storagerates.resourceType',
    'horizon.dashboard.container.storagerates.service'
  ];

  function run(registry, zun, basePath, resourceType, containerService) {
    registry.getResourceType(resourceType)
    .setNames(gettext('Account'), gettext('Storage Rates'))
  ...
  function containerProperties() {
    return {
      'id': {label: gettext('Storagerate ID'), filters: ['noValue'] },
      'storage_size': {label: gettext('Storage Size'), filters: ['noValue'] },
      ...
      'storage_rate': {label: gettext('Storage Rate'), filters: ['noValue'] },
  ...
  function config($provide, $windowProvider, $routeProvider) {
    var path = $windowProvider.$get().STATIC_URL + 'dashboard/container/storagerates/';
    $provide.constant('horizon.dashboard.container.storagerates.basePath', path);
    $routeProvider.when('/admin/container/storagerates', {
