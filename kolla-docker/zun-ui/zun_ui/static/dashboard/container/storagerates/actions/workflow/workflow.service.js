...
(function() {
  ...
  angular
    ...
    .factory("horizon.dashboard.container.storagerates.workflow", workflow);
  ...
  function workflow(zun, neutron, basePath, gettext, treeService) {
    ...
    function init(action, title, submitText) {
      ...
      if(displaynamesList.length == 0){
        queryProviderregions();
      }

      function queryProviderregions(){
        zun.getProviderregions().then(modifyProviderregionsResponse);

        function modifyProviderregionsResponse(response) {
      ...
      schema = {
        ...
        properties: {
          // info
          id: {
            title: gettext("Storagerate ID"),
            type: "string"
          },
          storage_size: {
            title: gettext("Storage Size"),
            type: "string"
          },
          provider_region_id: {
            title: gettext("Provider Region ID"),
            type: "string"
          },
          start_time: {
          ...
          storage_rate: {
            title: gettext("Storage Rate"),
            type: "string"
          },
          enable_ind: {
            title: gettext("Enable IND"),
            type: "string"
          },
          user_tier: {
      ...
      form = [
        {
          ...
          tabs: [
            {
              title: gettext("Storage Rates"),
              help: basePath + "storagerates/actions/workflow/info.help.html",
              type: "section",
              htmlClass: "row",
              items: [
                {
                  ...
                  items: [
                    {
                      key: "storage_size",
                      placeholder: gettext("name of the storagerate account"),
