...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.storagerates.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.storagerates.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.storagerates.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete storage rates: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Storage Rate',
                        'Confirm Delete Storage Rates', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted storage rate is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted storage rates are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Storage Rate',
                         'Delete Storage Rates', count),
        success: ngettext('Deleted Storage Rate: %s.',
                          'Deleted Storage Rates: %s.', count),
        error: ngettext('Unable to delete Storage Rate: %s.',
                        'Unable to delete Storage Rates: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/storagerates(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/storagerates');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteStoragerate(id, true);
    }
  }
})();
