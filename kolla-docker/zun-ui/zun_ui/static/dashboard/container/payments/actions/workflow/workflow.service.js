...
(function() {
  ...
  angular
    ...
    .factory("horizon.dashboard.container.payments.workflow", workflow);
  ...
  function workflow(zun, neutron, basePath, gettext, treeService) {
    ...
    function init(action, title, submitText) {
      ...
      function queryStatements(){
        zun.getStatements().then(modifyStatementsResponse);

        function modifyStatementsResponse(response) {
      ...
      function queryPaymentmethods(){
        zun.getPaymentmethods().then(modifyPaymentmethodsResponse);

        function modifyPaymentmethodsResponse(response) {
            var res = {data: {items: response.data.items.map(modifyItem)}};
            var data = res["data"];
            var items = data["items"];
            var array = [];
      ...
      schema = {
        ...
        properties: {
          ...
          payment_method_id: {
            title: gettext("Payment Method ID"),
            type: "string"
          },
          amount: {
          ...
          tabs: [
            {
              ...
              help: basePath + "payments/actions/workflow/info.help.html",
              type: "section",
              htmlClass: "row",
              items: [
                {
