...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.payments.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.payments.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.payments.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete payments: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Payment',
                        'Confirm Delete Payments', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted payment is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted payments are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Payment',
                         'Delete Payments', count),
        success: ngettext('Deleted Payment: %s.',
                          'Deleted Payments: %s.', count),
        error: ngettext('Unable to delete Payment: %s.',
                        'Unable to delete Payments: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/payments(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/payments');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deletePayment(id, true);
    }
  }
})();
