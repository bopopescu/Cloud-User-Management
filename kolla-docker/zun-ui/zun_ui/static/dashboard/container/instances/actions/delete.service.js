...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.instances.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.instances.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.instances.events',
  ...
  function deleteService(
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Instance',
                        'Confirm Delete Instances', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted instance is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted instances are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Instance',
                         'Delete Instances', count),
        success: ngettext('Deleted Instance: %s.',
                          'Deleted Instances: %s.', count),
        error: ngettext('Unable to delete Instance: %s.',
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/instances(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/instances');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteInstance(id, true);
    }
  }
})();
