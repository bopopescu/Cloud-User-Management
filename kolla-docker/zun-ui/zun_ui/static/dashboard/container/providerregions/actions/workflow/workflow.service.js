...
(function() {
  ...
  angular
    ...
    .factory("horizon.dashboard.container.providerregions.workflow", workflow);
  ...
  function workflow(zun, neutron, basePath, gettext, treeService) {
    ...
    function init(action, title, submitText) {
      ...
      if(displaynamesList.length == 0){
        queryProviders();
        //alert(JSON.stringify(c)+'1');
      }

      function queryProviders(){
        zun.getProviders().then(modifyProvidersResponse);
        //alert(JSON.stringify(c)+'2');
        //return c;

        function modifyProvidersResponse(response) {
            ...
            form[0].tabs[0].items[1].items[0].titleMap = array;
      ...
      schema = {
        ...
        properties: {
          // info
          id: {
            title: gettext("Providerregion ID"),
            type: "string"
          },
          region: {
          ...
          tabs: [
            {
              ...
              help: basePath + "providerregions/actions/workflow/info.help.html",
              type: "section",
              htmlClass: "row",
              items: [
                /*{
                  ...
                  items: [
                    {
                      key: "provider_id",
                      placeholder: gettext("name of the providerregion account"),
                      required: true
                    }
                  ]
                },*/
