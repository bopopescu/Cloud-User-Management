...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.providerregions.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.providerregions.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.providerregions.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete provider regions: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Provider Region',
                        'Confirm Delete Provider Regions', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted provider region is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted provider regions are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Provider Region',
                         'Delete Provider Regions', count),
        success: ngettext('Deleted Provider Region: %s.',
                          'Deleted Provider Regions: %s.', count),
        error: ngettext('Unable to delete Provider Region: %s.',
                        'Unable to delete Provider Regions: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/providerregions(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/providerregions');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteProviderregion(id, true);
    }
  }
})();
