...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.statements.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.statements.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.statements.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete statements: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Statement',
                        'Confirm Delete Statements', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted statement is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted statements are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Statement',
                         'Delete Statements', count),
        success: ngettext('Deleted Statement: %s.',
                          'Deleted Statements: %s.', count),
        error: ngettext('Unable to delete Statement: %s.',
                        'Unable to delete Statements: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/statements(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/statements');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteStatement(id, true);
    }
  }
})();
