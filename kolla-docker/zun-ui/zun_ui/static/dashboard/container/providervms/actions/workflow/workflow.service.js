...
(function() {
  ...
  angular
    ...
    .factory("horizon.dashboard.container.providervms.workflow", workflow);
  ...
  function workflow(zun, neutron, basePath, gettext, treeService) {
    ...
    function init(action, title, submitText) {
      ...
      if(displaynamesList.length == 0){
        queryProvideraccounts();
      }

      function queryProvideraccounts(){
        zun.getProvideraccounts().then(modifyProvideraccountsResponse);

        function modifyProvideraccountsResponse(response) {
            ...
            for(i=0; i<items.length; i++){
                ...
                var uuid = items[i]["id"];
                var obj = {
                      value: uuid + '~' + gettext(displayname) + "_" + gettext(displayname2),
                      name: gettext(displayname) + "_" + gettext(displayname2)
                };
                array.push(obj);
      ...
      schema = {
        ...
        properties: {
          ...
          provider_account_id: {
            title: gettext("Provider Account ID"),
            type: "string"
          },
          vm_external_ipv4: {
            title: gettext("VM External Ipv4"),
            type: "string"
          },
          vm_internal_ipv4: {
            title: gettext("VM Internal Ipv4"),
            type: "string"
          },
          status: {
      ...
      form = [
        {
          ...
          tabs: [
            {
              title: gettext("Provider VMs"),
              help: basePath + "providervms/actions/workflow/info.help.html",
              type: "section",
              htmlClass: "row",
              items: [
                {
