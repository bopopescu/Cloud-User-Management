...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.providervms.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.providervms.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.providervms.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete provider vms: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Provider VM',
                        'Confirm Delete Provider VMs', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted provider vm is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted provider vms are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Provider VM',
                         'Delete Provider VMs', count),
        success: ngettext('Deleted Provider VM: %s.',
                          'Deleted Provider VMs: %s.', count),
        error: ngettext('Unable to delete Provider VM: %s.',
                        'Unable to delete Provider VMs: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/providervms(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/providervms');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteProvidervm(id, true);
    }
  }
})();
