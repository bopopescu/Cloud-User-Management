...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.instancetypes.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.instancetypes.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.instancetypes.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete instance types: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Instance Type',
                        'Confirm Delete Instance Types', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted instance type is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted instance types are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Instance Type',
                         'Delete Instance Types', count),
        success: ngettext('Deleted Instance Type: %s.',
                          'Deleted Instance Types: %s.', count),
        error: ngettext('Unable to delete Instance Type: %s.',
                        'Unable to delete Instance Types: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/instancetypes(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/instancetypes');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteInstancetype(id, true);
    }
  }
})();
