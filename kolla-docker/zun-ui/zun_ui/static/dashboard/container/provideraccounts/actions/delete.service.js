...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.provideraccounts.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.provideraccounts.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.provideraccounts.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete provider accounts: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Provider Account',
                        'Confirm Delete Provider Accounts', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted provider account is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted provider accounts are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Provider Account',
                         'Delete Provider Accounts', count),
        success: ngettext('Deleted Provider Account: %s.',
                          'Deleted Provider Accounts: %s.', count),
        error: ngettext('Unable to delete Provider Account: %s.',
                        'Unable to delete Provider Accounts: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/provideraccounts(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/provideraccounts');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteProvideraccount(id, true);
    }
  }
})();
