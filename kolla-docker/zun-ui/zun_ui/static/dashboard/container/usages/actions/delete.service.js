...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.usages.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.usages.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.usages.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete usages: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Usage',
                        'Confirm Delete Usages', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted usage is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted usages are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Usage',
                         'Delete Usages', count),
        success: ngettext('Deleted Usage: %s.',
                          'Deleted Usages: %s.', count),
        error: ngettext('Unable to delete Usage: %s.',
                        'Unable to delete Usages: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/usages(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/usages');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteUsage(id, true);
    }
  }
})();
