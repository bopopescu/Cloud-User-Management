...
(function() {
  ...
  angular
    ...
    .factory("horizon.dashboard.container.usages.workflow", workflow);
  ...
  function workflow(zun, neutron, basePath, gettext, treeService) {
    ...
    function init(action, title, submitText) {
      ...
      function queryInstances(){
        zun.getInstances().then(modifyInstancesResponse);

        function modifyInstancesResponse(response) {
            var res = {data: {items: response.data.items.map(modifyItem)}};
            var data = res["data"];
            var items = data["items"];
            var array = [];
            ...
            for(i=0; i<items.length; i++){
                var displayname = items[i]["displayname"];
                var displayname2 = items[i]["displayname2"];
                var displayname3 = items[i]["displayname3"];
                var uuid = items[i]["id"];
                var obj = {
                      value: uuid + '~' + gettext(displayname) + "_" + gettext(displayname2) + "_" + gettext(displayname3),
                      name: gettext(displayname) + "_" + gettext(displayname2) + "_" + gettext(displayname3)
                };
                array.push(obj);
                // alert(JSON.stringify(array));
            }
      ...
      function queryComputerates(){
        zun.getComputerates().then(modifyComputeratesResponse);

        function modifyComputeratesResponse(response) {
            var res = {data: {items: response.data.items.map(modifyItem)}};
            var data = res["data"];
            var items = data["items"];
            var array = [];
            ...
            form[0].tabs[0].items[6].items[0].titleMap = array;
      ...
      //var displaynamesList4 = [];
      if(displaynamesList4.length == 0){
        queryStoragerates();
      }

      function queryStoragerates(){
        zun.getStoragerates().then(modifyStorageratesResponse);

        function modifyStorageratesResponse(response) {
            var res = {data: {items: response.data.items.map(modifyItem)}};
            var data = res["data"];
            var items = data["items"];
            var array = [];
            ...
            form[0].tabs[0].items[7].items[0].titleMap = array;
            //displaynamesList4 = array;
            // alert(JSON.stringify(displaynamesList) + "hahahahaha!");
      ...
      schema = {
        ...
        properties: {
          ...
          compute_rate_id: {
            title: gettext("Compute Rate ID"),
            type: "string"
          },
          storage_rate_id: {
            title: gettext("Storage Rate ID"),
            type: "string"
          },
          start_time: {
      ...
      form = [
        {
          ...
          tabs: [
            {
              title: gettext("Usages"),
              help: basePath + "usages/actions/workflow/info.help.html",
              type: "section",
              htmlClass: "row",
              items: [
                {
