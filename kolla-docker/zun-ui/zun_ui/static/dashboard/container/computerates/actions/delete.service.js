...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.computerates.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.computerates.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.computerates.events',
    'horizon.dashboard.container.computerates.validStates'
  ];
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete compute rates: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Compute Rate',
                        'Confirm Delete Compute Rates', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted compute rate is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted compute rates are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Compute Rate',
                         'Delete Compute Rates', count),
        success: ngettext('Deleted Compute Rate: %s.',
                          'Deleted Compute Rates: %s.', count),
        error: ngettext('Unable to delete Compute Rate: %s.',
                        'Unable to delete Compute Rates: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/computerates(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/computerates');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteComputerate(id, true);
    }
  }
})();
