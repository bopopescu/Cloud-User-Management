...
(function() {
  ...
  /**
   * @ngdoc overview
   * @name horizon.dashboard.container.computerates.update.service
   * @description Service for the container update modal
   */
  angular
    .module('horizon.dashboard.container.computerates')
    .factory('horizon.dashboard.container.computerates.update.service', updateService);
  ...
  updateService.$inject = [
    ...
    'horizon.dashboard.container.computerates.workflow',
  ...
  function updateService(
    ...
    function perform(selected) {
      ...
      function onLoad(response) {
        ...
        config.model.compute_rate = response.data.compute_rate
          ? response.data.compute_rate : "";
        ...
        var res = response.data.instance_type_id + "~" + arr[1] + "_" + arr[2] + "_" + arr[3];
    ...
    function submit(context) {
      ...
      context.model.instance_type_id=arr[0];
      context.model.displayname=context.model.compute_rate+"_"+arr[1];
      context.model = cleanUpdateProperties(context.model);
      return zun.updateComputerate(id, context.model).then(success);
    }
    ...
    function cleanUpdateProperties(model) {
      ...
      for (var key in model) {
        if (model.hasOwnProperty(key) && model[key] === null || model[key] === "" ||
            (key !== "displayname" && key !== "instance_type_id" && key !== "start_date" && key !== "end_time" && key !== "compute_rate" && key !== "status" && key !== "user_tier")) {
          delete model[key];
        }
      }
      return model;
