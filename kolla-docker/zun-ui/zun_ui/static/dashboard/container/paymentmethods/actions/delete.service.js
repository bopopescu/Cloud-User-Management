...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.paymentmethods.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.paymentmethods.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.paymentmethods.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete payment methods: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete Payment Method',
                        'Confirm Delete Payment Methods', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted payment method is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted payment methods are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete Payment Method',
                         'Delete Payment Methods', count),
        success: ngettext('Deleted Payment Method: %s.',
                          'Deleted Payment Methods: %s.', count),
        error: ngettext('Unable to delete Payment Method: %s.',
                        'Unable to delete Payment Methods: %s.', count)
      };
    }
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/paymentmethods(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/paymentmethods');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deletePaymentmethod(id, true);
    }
  }
})();
