...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.users.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.users.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.users.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete users: %s");
    ...
    function labelize(count) {
      return {
        title: ngettext('Confirm Delete User',
                        'Confirm Delete Users', count),
        /* eslint-disable max-len */
        message: ngettext('You have selected "%s". Please confirm your selection. Deleted user is not recoverable.',
                          'You have selected "%s". Please confirm your selection. Deleted users are not recoverable.', count),
        /* eslint-enable max-len */
        submit: ngettext('Delete User',
                         'Delete Users', count),
        success: ngettext('Deleted User: %s.',
                          'Deleted Users: %s.', count),
        error: ngettext('Unable to delete User: %s.',
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/users(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/users');
        }
      } else {
