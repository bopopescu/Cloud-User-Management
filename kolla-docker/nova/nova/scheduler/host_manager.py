...
class HostManager(object):
    ...
    def _get_computes_for_cells(self, context, cells, compute_uuids=None):
        ...
        services = {}
        LOG.debug('xxx _get_computes_for_cells start v3333= %(node)s',
                  {'node': compute_nodes})
        for cell in cells:
            LOG.debug('Getting compute nodes and services for cell %(cell)s',
                      {'cell': cell.identity})
            with context_module.target_cell(context, cell) as cctxt:
                LOG.debug('xxx inside  compute nodes and services for compute_uuids %(cell)s',
                          {'cell': compute_uuids})
                LOG.debug('xxx inside  compute nodes and services for cctxt %(cell)s',
                          {'cell': cctxt})
                LOG.debug('xxx inside  compute nodes and services for ComputeNodeList %(cell)s',
                         {'cell': objects.ComputeNodeList.get_all_by_uuids(
                            cctxt, compute_uuids)})
        ...
        LOG.debug('_get_computes_for_cells %(node)s',
                  {'node': compute_nodes})
