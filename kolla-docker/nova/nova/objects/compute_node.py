...
class ComputeNodeList(base.ObjectListBase, base.NovaObject):
    ...
    def _db_compute_node_get_all_by_uuids(context, compute_uuids):
        xxx=context.session.query(models.ComputeNode)
        LOG.debug('xxx inside  _db_compute_node_get_all_by_uuids session ComputeNode %(cell)s',
                  {'cell': xxx})
        LOG.debug('xxx inside  _db_compute_node_get_all_by_uuids session ComputeNode uuid %(cell)s',
                  {'cell': models.ComputeNode.uuid.in_(compute_uuids)})
        db_computes = context.session.query(models.ComputeNode).filter(
            models.ComputeNode.uuid.in_(compute_uuids)).all()
        LOG.debug('xxx inside  _db_compute_node_get_all_by_uuids session db_computes %(cell)s',
                  {'cell': db_computes})
        return db_computes
