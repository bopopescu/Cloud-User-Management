...
CREATION_ATTRIBUTES = ['id', 'storage_size', 'provider_region_id', 'start_time', 'end_time', 'storage_rate', 'enable_ind', 'user_tier', 'displayname',
...
class Storagerate(base.Resource):
    def __repr__(self):
        return "<Storagerate %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Storagerate

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/storagerates/%s' % id
        else:
            return '/v1/storagerates'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient storagerate storagerate path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
