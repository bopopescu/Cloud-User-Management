...
CREATION_ATTRIBUTES = ['id', 'user_id', 'previous_balance', 'billing_begin_date', 'billing_end_date', 'billing_charge_amount', 'current_balance', 'displayname',
...
class Statement(base.Resource):
    def __repr__(self):
        return "<Statement %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Statement

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/statements/%s' % id
        else:
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient statement statement path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
