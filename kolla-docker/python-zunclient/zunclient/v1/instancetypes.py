...
CREATION_ATTRIBUTES = ['id', 'memory_size', 'no_of_cpu', 'provider_region_id', 'enable_ind', 'start_date', 'end_date', 'displayname',
...
class Instancetype(base.Resource):
    def __repr__(self):
        return "<Instancetype %s>" % self._info
...
class ContainerManager(base.Manager):
    ...
    def _path(id=None):
        ...
        if id:
            return '/v1/instancetypes/%s' % id
        else:
            return '/v1/instancetypes'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient instancetype instancetype path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
