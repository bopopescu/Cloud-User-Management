...
CREATION_ATTRIBUTES = ['id', 'create_date', 'provider_account_id', 'vm_external_ipv4', 'vm_internal_ipv4', 'status', 'displayname',
...
class Providervm(base.Resource):
    def __repr__(self):
        return "<Providervm %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Providervm

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/providervms/%s' % id
        else:
            return '/v1/providervms'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient providervm providervm path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
