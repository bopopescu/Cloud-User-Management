...
CREATION_ATTRIBUTES = ['id', 'user_id', 'provider_id', 'access_key_id', 'access_key_str', 'displayname', 'displayname2',
...
class Provideraccount(base.Resource):
    def __repr__(self):
        return "<Provideraccount %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Provideraccount

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/provideraccounts/%s' % id
        else:
            return '/v1/provideraccounts'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient provideraccount provideraccount path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
