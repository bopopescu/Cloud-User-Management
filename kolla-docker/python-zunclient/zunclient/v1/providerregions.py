...
CREATION_ATTRIBUTES = ['id', 'provider_id', 'region', 'displayname',
...
class Providerregion(base.Resource):
    def __repr__(self):
        return "<Providerregion %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Providerregion

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/providerregions/%s' % id
        else:
            return '/v1/providerregions'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient providerregion providerregion path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
