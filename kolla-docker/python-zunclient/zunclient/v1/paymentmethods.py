...
CREATION_ATTRIBUTES = ['id', 'user_id', 'payment_method_type', 'cc_first_name', 'cc_middle_name', 'cc_last_name', 'cc_card_no',
                       'cc_billing_address_line1', 'cc_billing_address_line2', 'cc_billing_address_line3', 'cc_billing_address_apt_suite_no',
                       'cc_city', 'cc_state', 'cc_zipcode', 'cc_country', 'cc_expiration_date', 'pp_email', 'displayname', 'cc_cvv',
...
class Paymentmethod(base.Resource):
    def __repr__(self):
        return "<Paymentmethod %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Paymentmethod

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/paymentmethods/%s' % id
        else:
            return '/v1/paymentmethods'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient paymentmethod paymentmethod path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
