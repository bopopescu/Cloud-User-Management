...
CREATION_ATTRIBUTES = ['id', 'instance_type_id', 'start_date', 'end_time', 'compute_rate', 'status', 'user_tier', 'displayname',
...
class Computerate(base.Resource):
    def __repr__(self):
        return "<Computerate %s>" % self._info


class ContainerManager(base.Manager):
    resource_class = Computerate

    @staticmethod
    def _path(id=None):

        if id:
            return '/v1/computerates/%s' % id
        else:
            return '/v1/computerates'
    ...
    def list(self, marker=None, limit=None, sort_key=None,
        ...
        LOG.debug('zunclient computerate computerate path=%s,  xxx list args xxx yyyy zzzzzzzzzzz', self._path(path))
        if limit is None:
