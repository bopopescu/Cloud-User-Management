...
class Site(Registry, HorizonComponent):
    ...
    def get_user_home(self, user):
        ...
        LOG.info("xxx enter get_user_home of horizon/base.py ")
        ...
        LOG.info("xxx user_home")
        LOG.info(user_home)

        if user_home:
            if callable(user_home):

                LOG.info("xxx user_home(user)")
                LOG.info(user_home(user))

                return user_home(user)
            elif isinstance(user_home, six.string_types):

                LOG.info("xxx enter elif isinstance")
                ...
                if '/' in user_home:
                    ...
                    LOG.info("xxx getattr()")
                    LOG.info(import_module(mod))
                    LOG.info(func)
                    LOG.info(user)

                    LOG.info(getattr(import_module(mod), func)(user))
    ...
    '''def get_index_home(self, user):
        LOG.info("xxx enter get_index_home of horizon/base.py xxx")
        return "/"'''
