...
class HorizonMiddleware(object):
    ...
    def process_exception(self, request, exception):
        ...
        """
        LOG.info("xxx into process_exception xxx")
        if isinstance(exception, (exceptions.NotAuthorized,
            ...
            LOG.info("xxx before redirect_to_login xxx")
            ...
            LOG.info("auth_url is")
            LOG.info(auth_url)

            LOG.info("next_url is")
            LOG.info(next_url)

            LOG.info("login_url is")
            LOG.info(login_url)

            LOG.info("response in process_exception is")
            LOG.info(response)

            LOG.info("field_name is")
            LOG.info(field_name)

            if isinstance(exception, exceptions.NotAuthorized):
                LOG.info("enter if isinstance(exception, exceptions.NotAuthorized):")
                response.delete_cookie('messages')
                return shortcuts.render(request, 'not_authorized.html',
                                        status=403)

            if request.is_ajax():
                LOG.info("enter if request.is_ajax():")
                response_401 = http.HttpResponse(status=401)
                response_401['X-Horizon-Location'] = response['location']
                return response_401

            LOG.info("just before process_exception response return")
