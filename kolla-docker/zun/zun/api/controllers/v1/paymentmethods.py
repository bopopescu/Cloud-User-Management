...
from zun.api.controllers.v1.views import paymentmethods_view as view
...
def _get_container(container_id):
    container = api_utils.get_resource('Paymentmethod', container_id)
    if not container:
...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('paymentmethod get_all xxx kwargs=%s', kwargs)
        context = pecan.request.context
        policy.enforce(context, "paymentmethod:get_all",
                       action="paymentmethod:get_all")
    ...
    def _get_containers_collection(self, **kwargs):
        ...
        if is_all_tenants(kwargs):
            ...
            policy.enforce(context, "paymentmethod:get_all_all_tenants",
                           action="paymentmethod:get_all_all_tenants")
    ...
    def get_one(self, container_id, **kwargs):
        ...
        """
        LOG.debug('paymentmethod get_one xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "paymentmethod:get_one_all_tenants",
                           action="paymentmethod:get_one_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "paymentmethod:get_one")
        compute_api = pecan.request.compute_api
        container = compute_api.paymentmethod_show(context, container)
        LOG.debug('paymentmethod get_one xxx host_url=%s, dict=%s', pecan.request.host_url, container.__dict__)
    ...
    #@validation.validated(schema.paymentmethod_create)
    def post(self, run=False, **container_dict):
        ...
        """
        LOG.debug('paymentmethod post xxx run=%s, container_dict=%s', run, container_dict)
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        #LOG.debug('container py post xxx context=%s,compute_api=%s' % (context.to_dict(),dir(compute_api)))
        policy.enforce(context, "paymentmethod:create",
                       action="paymentmethod:create")

        requested_networks = {}
        extra_spec = container_dict.get('hints', None)

        LOG.info("pppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp")
        #for key, value in context.items():
            #LOG.info('{} => {}'.format(key, value))
        LOG.info(context.user_id)
        LOG.info(context.user_name)
        LOG.info(container_dict)

        # container_dict['user_id'] = context.user_id

        if context.user_name == "admin" or context.user_id == container_dict['user_id']:
            new_paymentmethod = objects.Paymentmethod(context, **container_dict)
            new_paymentmethod.create(context)
            compute_api.paymentmethod_create(context, new_paymentmethod, extra_spec,
                                         requested_networks)
            # Set the HTTP Location Header
            pecan.response.location = link.build_url('paymentmethods',
                                                     new_paymentmethod.uuid)
            pecan.response.status = 202
            LOG.debug('paymentmethod post xxx pecan.response.status=%s', pecan.response.status)
            return view.format_container(pecan.request.host_url, new_paymentmethod)
        else:
            return None

    @pecan.expose('json')
    @exception.wrap_pecan_controller_exception
 # disable validation #   @validation.validated(schema.paymentmethod_update)
    def patch(self, container_id, **patch):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "paymentmethod:update")
       #utils.validate_container_state(container, 'update')
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        container = compute_api.paymentmethod_update(context, container, patch)
        return view.format_container(pecan.request.host_url, container)
    ...
    def rename(self, container_id, name):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "paymentmethod:rename")
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.1 -1.6 Paymentmethod delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "paymentmethod:delete_all_tenants",
                           action="paymentmethod:delete_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "paymentmethod:delete")
        try:
        ...
        compute_api = pecan.request.compute_api
        compute_api.paymentmethod_delete(context, container, force)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.7 Paymentmethod delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "paymentmethod:delete_all_tenants",
                           action="paymentmethod:delete_all_tenants")
        ...
        if not force:
            ...
            policy.enforce(context, "paymentmethod:delete_force",
                           action="paymentmethod:delete_force")
        compute_api = pecan.request.compute_api
        compute_api.paymentmethod_delete(context, container, force)
        pecan.response.status = 204
