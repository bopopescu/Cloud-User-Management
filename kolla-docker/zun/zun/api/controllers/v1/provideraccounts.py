...
from zun.api.controllers.v1.views import provideraccounts_view as view
...
def _get_container(container_id):
    container = api_utils.get_resource('Provideraccount', container_id)
    if not container:
...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('provideraccount get_all xxx kwargs=%s', kwargs)
        context = pecan.request.context
        policy.enforce(context, "provideraccount:get_all",
                       action="provideraccount:get_all")
    ...
    def _get_containers_collection(self, **kwargs):
        ...
        if is_all_tenants(kwargs):
            ...
            policy.enforce(context, "provideraccount:get_all_all_tenants",
                           action="provideraccount:get_all_all_tenants")
    ...
    def get_one(self, container_id, **kwargs):
        ...
        """
        LOG.debug('provideraccount get_one xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "provideraccount:get_one_all_tenants",
                           action="provideraccount:get_one_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "provideraccount:get_one")
        compute_api = pecan.request.compute_api
        container = compute_api.provideraccount_show(context, container)
        LOG.debug('provideraccount get_one xxx host_url=%s, dict=%s', pecan.request.host_url, container.__dict__)
    ...
    #@validation.validated(schema.provideraccount_create)
    def post(self, run=False, **container_dict):
        ...
        """
        LOG.debug('provideraccount post xxx run=%s, container_dict=%s', run, container_dict)
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        #LOG.debug('container py post xxx context=%s,compute_api=%s' % (context.to_dict(),dir(compute_api)))
        policy.enforce(context, "provideraccount:create",
                       action="provideraccount:create")

        requested_networks = {}
        extra_spec = container_dict.get('hints', None)
        new_provideraccount = objects.Provideraccount(context, **container_dict)
        new_provideraccount.create(context)
        compute_api.provideraccount_create(context, new_provideraccount, extra_spec,
                                         requested_networks)
        # Set the HTTP Location Header
        pecan.response.location = link.build_url('provideraccounts',
                                                     new_provideraccount.uuid)
        pecan.response.status = 202
        LOG.debug('provideraccount post xxx pecan.response.status=%s', pecan.response.status)
        return view.format_container(pecan.request.host_url, new_provideraccount)

    @pecan.expose('json')
    @exception.wrap_pecan_controller_exception
 # disable validation #   @validation.validated(schema.provideraccount_update)
    def patch(self, container_id, **patch):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "provideraccount:update")
       #utils.validate_container_state(container, 'update')
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        container = compute_api.provideraccount_update(context, container, patch)
        return view.format_container(pecan.request.host_url, container)
    ...
    def rename(self, container_id, name):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "provideraccount:rename")
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.1 -1.6 Provideraccount delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "provideraccount:delete_all_tenants",
                           action="provideraccount:delete_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "provideraccount:delete")
        try:
        ...
        compute_api = pecan.request.compute_api
        compute_api.provideraccount_delete(context, container, force)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.7 Provideraccount delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "provideraccount:delete_all_tenants",
                           action="provideraccount:delete_all_tenants")
        ...
        if not force:
            ...
            policy.enforce(context, "provideraccount:delete_force",
                           action="provideraccount:delete_force")
        compute_api = pecan.request.compute_api
        compute_api.provideraccount_delete(context, container, force)
        pecan.response.status = 204
