...
from zun.api.controllers.v1.views import storagerates_view as view
...
def _get_container(container_id):
    container = api_utils.get_resource('Storagerate', container_id)
    if not container:
...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('storagerate get_all xxx kwargs=%s', kwargs)
        context = pecan.request.context
        policy.enforce(context, "storagerate:get_all",
                       action="storagerate:get_all")
    ...
    def _get_containers_collection(self, **kwargs):
        ...
        if is_all_tenants(kwargs):
            ...
            policy.enforce(context, "storagerate:get_all_all_tenants",
                           action="storagerate:get_all_all_tenants")
    ...
    def get_one(self, container_id, **kwargs):
        ...
        """
        LOG.debug('storagerate get_one xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "storagerate:get_one_all_tenants",
                           action="storagerate:get_one_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "storagerate:get_one")
        compute_api = pecan.request.compute_api
        container = compute_api.storagerate_show(context, container)
        LOG.debug('storagerate get_one xxx host_url=%s, dict=%s', pecan.request.host_url, container.__dict__)
    ...
    #@validation.validated(schema.storagerate_create)
    def post(self, run=False, **container_dict):
        ...
        """
        LOG.debug('storagerate post xxx run=%s, container_dict=%s', run, container_dict)
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        #LOG.debug('container py post xxx context=%s,compute_api=%s' % (context.to_dict(),dir(compute_api)))
        policy.enforce(context, "storagerate:create",
                       action="storagerate:create")

        requested_networks = {}
        extra_spec = container_dict.get('hints', None)
        new_storagerate = objects.Storagerate(context, **container_dict)
        new_storagerate.create(context)
        compute_api.storagerate_create(context, new_storagerate, extra_spec,
                                         requested_networks)
        # Set the HTTP Location Header
        pecan.response.location = link.build_url('storagerates',
                                                     new_storagerate.uuid)
        pecan.response.status = 202
        LOG.debug('storagerate post xxx pecan.response.status=%s', pecan.response.status)
        return view.format_container(pecan.request.host_url, new_storagerate)

    @pecan.expose('json')
    @exception.wrap_pecan_controller_exception
 # disable validation #   @validation.validated(schema.storagerate_update)
    def patch(self, container_id, **patch):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "storagerate:update")
       #utils.validate_container_state(container, 'update')
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        container = compute_api.storagerate_update(context, container, patch)
        return view.format_container(pecan.request.host_url, container)
    ...
    def rename(self, container_id, name):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "storagerate:rename")
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.1 -1.6 Storagerate delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "storagerate:delete_all_tenants",
                           action="storagerate:delete_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "storagerate:delete")
        try:
        ...
        compute_api = pecan.request.compute_api
        compute_api.storagerate_delete(context, container, force)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.7 Storagerate delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "storagerate:delete_all_tenants",
                           action="storagerate:delete_all_tenants")
        ...
        if not force:
            ...
            policy.enforce(context, "storagerate:delete_force",
                           action="storagerate:delete_force")
        compute_api = pecan.request.compute_api
        compute_api.storagerate_delete(context, container, force)
        pecan.response.status = 204
