...
from zun.api.controllers.v1.views import providers_view as view
...
def _get_container(container_id):
    container = api_utils.get_resource('Provider', container_id)
    if not container:
...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('provider get_all xxx kwargs=%s', kwargs)
        context = pecan.request.context
        policy.enforce(context, "provider:get_all",
                       action="provider:get_all")
    ...
    def _get_containers_collection(self, **kwargs):
        ...
        if is_all_tenants(kwargs):
            ...
            policy.enforce(context, "provider:get_all_all_tenants",
                           action="provider:get_all_all_tenants")
    ...
    #@validation.validated(schema.provider_create)
    ...
    @pecan.expose('json')
    @exception.wrap_pecan_controller_exception
 # disable validation #   @validation.validated(schema.provider_update)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.1 -1.6 Provider delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "provider:delete_all_tenants",
                           action="provider:delete_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "provider:delete")
        try:
        ...
        compute_api = pecan.request.compute_api
        compute_api.provider_delete(context, container, force)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.7 Provider delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "provider:delete_all_tenants",
                           action="provider:delete_all_tenants")
        ...
        if not force:
            ...
            policy.enforce(context, "provider:delete_force",
                           action="provider:delete_force")
        compute_api = pecan.request.compute_api
        compute_api.provider_delete(context, container, force)
        pecan.response.status = 204
