...
from zun.api.controllers.v1.views import providervms_view as view
...
def _get_container(container_id):
    container = api_utils.get_resource('Providervm', container_id)
    if not container:
...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('providervm get_all xxx kwargs=%s', kwargs)
        context = pecan.request.context
        policy.enforce(context, "providervm:get_all",
                       action="providervm:get_all")
    ...
    def _get_containers_collection(self, **kwargs):
        ...
        if is_all_tenants(kwargs):
            ...
            policy.enforce(context, "providervm:get_all_all_tenants",
                           action="providervm:get_all_all_tenants")
    ...
    def get_one(self, container_id, **kwargs):
        ...
        """
        LOG.debug('providervm get_one xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "providervm:get_one_all_tenants",
                           action="providervm:get_one_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "providervm:get_one")
        compute_api = pecan.request.compute_api
        container = compute_api.providervm_show(context, container)
        LOG.debug('providervm get_one xxx host_url=%s, dict=%s', pecan.request.host_url, container.__dict__)
    ...
    #@validation.validated(schema.providervm_create)
    def post(self, run=False, **container_dict):
        ...
        """
        LOG.debug('providervm post xxx run=%s, container_dict=%s', run, container_dict)
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        #LOG.debug('container py post xxx context=%s,compute_api=%s' % (context.to_dict(),dir(compute_api)))
        policy.enforce(context, "providervm:create",
                       action="providervm:create")

        requested_networks = {}
        extra_spec = container_dict.get('hints', None)
        new_providervm = objects.Providervm(context, **container_dict)
        new_providervm.create(context)
        compute_api.providervm_create(context, new_providervm, extra_spec,
                                         requested_networks)
        # Set the HTTP Location Header
        pecan.response.location = link.build_url('providervms',
                                                     new_providervm.uuid)
        pecan.response.status = 202
        LOG.debug('providervm post xxx pecan.response.status=%s', pecan.response.status)
        return view.format_container(pecan.request.host_url, new_providervm)

    @pecan.expose('json')
    @exception.wrap_pecan_controller_exception
 # disable validation #   @validation.validated(schema.providervm_update)
    def patch(self, container_id, **patch):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "providervm:update")
       #utils.validate_container_state(container, 'update')
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        container = compute_api.providervm_update(context, container, patch)
        return view.format_container(pecan.request.host_url, container)
    ...
    def rename(self, container_id, name):
        ...
        """
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "providervm:rename")
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.1 -1.6 Providervm delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "providervm:delete_all_tenants",
                           action="providervm:delete_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "providervm:delete")
        try:
        ...
        compute_api = pecan.request.compute_api
        compute_api.providervm_delete(context, container, force)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.7 Providervm delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "providervm:delete_all_tenants",
                           action="providervm:delete_all_tenants")
        ...
        if not force:
            ...
            policy.enforce(context, "providervm:delete_force",
                           action="providervm:delete_force")
        compute_api = pecan.request.compute_api
        compute_api.providervm_delete(context, container, force)
        pecan.response.status = 204
