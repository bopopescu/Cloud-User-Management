...
from zun.api.controllers.v1.views import providerregions_view as view
...
def _get_container(container_id):
    container = api_utils.get_resource('Providerregion', container_id)
    if not container:
...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('providerregion get_all xxx kwargs=%s', kwargs)
        context = pecan.request.context
        policy.enforce(context, "providerregion:get_all",
                       action="providerregion:get_all")
    ...
    def _get_containers_collection(self, **kwargs):
        ...
        if is_all_tenants(kwargs):
            ...
            policy.enforce(context, "providerregion:get_all_all_tenants",
                           action="providerregion:get_all_all_tenants")
    ...
    def get_one(self, container_id, **kwargs):
        ...
        """
        LOG.debug('providerregion get_one xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "providerregion:get_one_all_tenants",
                           action="providerregion:get_one_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "providerregion:get_one")
        compute_api = pecan.request.compute_api
        container = compute_api.providerregion_show(context, container)
        LOG.debug('providerregion get_one xxx host_url=%s, dict=%s', pecan.request.host_url, container.__dict__)
    ...
    #@validation.validated(schema.providerregion_create)
    def post(self, run=False, **container_dict):
        ...
        """
        LOG.debug('providerregion post xxx run=%s, container_dict=%s', run, container_dict)
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        #LOG.debug('container py post xxx context=%s,compute_api=%s' % (context.to_dict(),dir(compute_api)))
        policy.enforce(context, "providerregion:create",
        ...
        compute_api.providerregion_create(context, new_providerregion, extra_spec,
                                         requested_networks)
        # Set the HTTP Location Header
        pecan.response.location = link.build_url('providerregions',
                                                     new_providerregion.uuid)
        pecan.response.status = 202
        LOG.debug('providerregion post xxx pecan.response.status=%s', pecan.response.status)
        return view.format_container(pecan.request.host_url, new_providerregion)

    @pecan.expose('json')
    @exception.wrap_pecan_controller_exception
 # disable validation #   @validation.validated(schema.providerregion_update)
    def patch(self, container_id, **patch):
       ...
       #utils.validate_container_state(container, 'update')
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        container = compute_api.providerregion_update(context, container, patch)
        return view.format_container(pecan.request.host_url, container)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.1 -1.6 Providerregion delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "providerregion:delete_all_tenants",
                           action="providerregion:delete_all_tenants")
            context.all_tenants = True
        container = _get_container(container_id)
        check_policy_on_container(container.as_dict(), "providerregion:delete")
        try:
        ...
        compute_api = pecan.request.compute_api
        compute_api.providerregion_delete(context, container, force)
    ...
    def delete(self, container_id, force=False, **kwargs):
        ...
        :param container_ident: UUID or Name of a container.
        """
        LOG.debug('v 1.7 Providerregion delete xxx container_id=%s, kwargs=%s', container_id, kwargs)
        context = pecan.request.context
        if is_all_tenants(kwargs):
            policy.enforce(context, "providerregion:delete_all_tenants",
                           action="providerregion:delete_all_tenants")
        ...
        if not force:
            ...
            policy.enforce(context, "providerregion:delete_force",
                           action="providerregion:delete_force")
        compute_api = pecan.request.compute_api
        compute_api.providerregion_delete(context, container, force)
        pecan.response.status = 204
