...
class Manager(periodic_task.PeriodicTasks):
    ...
    def user_update(self, context, container, patch):
        ...
        LOG.info("uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu")
        LOG.info(context)
        LOG.info(container)
        LOG.info(patch)
        ...
        if containers:
            for container in containers:
                try:
                    ...
                    return
    def _do_provideraccount_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_provideraccount_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_provideraccount_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_provideraccount_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def provideraccount_create(self, context, limits, requested_networks, container):
        self._do_provideraccount_create(context, container, requested_networks,limits, False)


    @translate_exception
    def provideraccount_delete(self, context, container, force):
        LOG.debug('Deleting provideraccount: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def provideraccount_show(self, context, container):
        LOG.debug('Showing provideraccount: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def provideraccount_update(self, context, container, patch):
        LOG.debug('Updating a provideraccount: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_provideraccounts(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Provideraccount.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting provideraccount '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.provideraccount_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_providervm_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_providervm_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_providervm_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_providervm_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def providervm_create(self, context, limits, requested_networks, container):
        self._do_providervm_create(context, container, requested_networks,limits, False)


    @translate_exception
    def providervm_delete(self, context, container, force):
        LOG.debug('Deleting providervm: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def providervm_show(self, context, container):
        LOG.debug('Showing providervm: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def providervm_update(self, context, container, patch):
        LOG.debug('Updating a providervm: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_providervms(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Providervm.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting providervm '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.providervm_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_instance_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_instance_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_instance_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_instance_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def instance_create(self, context, limits, requested_networks, container):
        self._do_instance_create(context, container, requested_networks,limits, False)


    @translate_exception
    def instance_delete(self, context, container, force):
        LOG.debug('Deleting instance: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def instance_show(self, context, container):
        LOG.debug('Showing instance: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def instance_update(self, context, container, patch):
        LOG.debug('Updating a instance: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_instances(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Instance.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting instance '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.instance_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_storagerate_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_storagerate_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_storagerate_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_storagerate_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def storagerate_create(self, context, limits, requested_networks, container):
        self._do_storagerate_create(context, container, requested_networks,limits, False)


    @translate_exception
    def storagerate_delete(self, context, container, force):
        LOG.debug('Deleting storagerate: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def storagerate_show(self, context, container):
        LOG.debug('Showing storagerate: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def storagerate_update(self, context, container, patch):
        LOG.debug('Updating a storagerate: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_storagerates(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Storagerate.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting storagerate '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.storagerate_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
        ...
        if containers:
            for container in containers:
                try:
                    ...
                    return
    def _do_providerregion_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_providerregion_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_providerregion_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_providerregion_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def providerregion_create(self, context, limits, requested_networks, container):
        self._do_providerregion_create(context, container, requested_networks,limits, False)


    @translate_exception
    def providerregion_delete(self, context, container, force):
        LOG.debug('Deleting providerregion: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def providerregion_show(self, context, container):
        LOG.debug('Showing providerregion: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def providerregion_update(self, context, container, patch):
        LOG.debug('Updating a providerregion: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_providerregions(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Providerregion.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting providerregion '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.providerregion_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_instancetype_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_instancetype_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_instancetype_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_instancetype_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def instancetype_create(self, context, limits, requested_networks, container):
        self._do_instancetype_create(context, container, requested_networks,limits, False)


    @translate_exception
    def instancetype_delete(self, context, container, force):
        LOG.debug('Deleting instancetype: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def instancetype_show(self, context, container):
        LOG.debug('Showing instancetype: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def instancetype_update(self, context, container, patch):
        LOG.debug('Updating a instancetype: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_instancetypes(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Instancetype.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting instancetype '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.instancetype_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_usage_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_usage_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_usage_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_usage_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def usage_create(self, context, limits, requested_networks, container):
        self._do_usage_create(context, container, requested_networks,limits, False)


    @translate_exception
    def usage_delete(self, context, container, force):
        LOG.debug('Deleting usage: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def usage_show(self, context, container):
        LOG.debug('Showing usage: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def usage_update(self, context, container, patch):
        LOG.debug('Updating a usage: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_usages(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Usage.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting usage '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.usage_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_statement_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_statement_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_statement_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_statement_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def statement_create(self, context, limits, requested_networks, container):
        self._do_statement_create(context, container, requested_networks,limits, False)


    @translate_exception
    def statement_delete(self, context, container, force):
        LOG.debug('Deleting statement: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def statement_show(self, context, container):
        LOG.debug('Showing statement: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def statement_update(self, context, container, patch):
        LOG.debug('Updating a statement: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_statements(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Statement.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting statement '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.statement_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_computerate_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_computerate_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_computerate_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_computerate_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def computerate_create(self, context, limits, requested_networks, container):
        self._do_computerate_create(context, container, requested_networks,limits, False)


    @translate_exception
    def computerate_delete(self, context, container, force):
        LOG.debug('Deleting computerate: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def computerate_show(self, context, container):
        LOG.debug('Showing computerate: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def computerate_update(self, context, container, patch):
        LOG.debug('Updating a computerate: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_computerates(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Computerate.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting computerate '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.computerate_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
                    ...
                    return
    def _do_payment_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_payment_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_payment_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_payment_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def payment_create(self, context, limits, requested_networks, container):
        self._do_payment_create(context, container, requested_networks,limits, False)


    @translate_exception
    def payment_delete(self, context, container, force):
        LOG.debug('Deleting payment: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    def payment_show(self, context, container):
        LOG.debug('Showing payment: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    def payment_update(self, context, container, patch):
        LOG.debug('Updating a payment: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_payments(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        containers = objects.Payment.list(context,
                                            filters=filters)

        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting payment '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.payment_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
