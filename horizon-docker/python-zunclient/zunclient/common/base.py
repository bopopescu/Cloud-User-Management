...
class Manager(object):
    ...
    def _list(self, url, response_key=None, obj_class=None, body=None,
        ...
        LOG.debug('zunclient _list url=%s, response_key=%s, obj_class=%s, qparams=%s', url, response_key, obj_class, qparams)
        resp, body = self.api.json_request('GET', url)
        LOG.debug('zunclient _list resp=%s, body=%s,', resp, body)
        if obj_class is None:
            obj_class = self.resource_class

        data = self._format_body_data(body, response_key)
        LOG.debug('zunclient _list data=%s,obj_class=%s', data, obj_class)
    ...
    def _delete(self, url, qparams=None):
        if qparams:
            url = "%s?%s" % (url, urlparse.urlencode(qparams))
        LOG.debug('zunclient _delete url=%s', url)
