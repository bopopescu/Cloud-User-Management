...
class ContainersController(base.Controller):
    ...
    def get_all(self, **kwargs):
        ...
        """
        LOG.debug('get_all xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx kwargs =%s  xxxxxxxxxxxxxxxxxxxxxxxxx XXXXXXXXXX.', kwargs)
        context = pecan.request.context
    ...
    def post(self, run=False, **container_dict):
        ...
        """
        LOG.debug('container post xxx run=%s, container_dict=%s', run, container_dict)
        context = pecan.request.context
        compute_api = pecan.request.compute_api
        ...
        action_type = container_dict.pop('action_type', None)
        if action_type is not None:
            LOG.debug('container py post xxx action_type=%s' % (action_type))
        else:
            msg = _('Invalid param action_type because current '
                    'action_type is %(action_type)s.') % \
                    {'action_type': action_type}
            raise exception.InvalidParam(msg)
        if action_type == "carrier":
            requested_networks = {}
