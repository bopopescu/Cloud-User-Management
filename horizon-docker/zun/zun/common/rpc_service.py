...
class Service(service.Service):
    ...
    def __init__(self, topic, server, endpoints, binary):
        ...
        #CONF.reset()
        LOG.debug('Service init xxx CONF=%s XXXXXXXXXX', CONF.__dict__)

        transport = messaging.get_rpc_transport(CONF)
        LOG.debug('Service init xxx transport=%s XXXXXXXXXX', transport.__str__)
...
class API(object):
    def __init__(self, transport=None, context=None, topic=None, server=None,
        ...
        if topic is None:
            topic = ''
        target = messaging.Target(topic=topic, server=server)
        LOG.debug('rpc client xxx transport=%s, target=%s,topic=%s, server=%s, XXXXXXXXXX', transport.__str__, target, topic, server)
    ...
    def _call(self, server, method, *args, **kwargs):
        ...
        LOG.debug('rpc service cctxt=%s, server=%s, method=%s, args=%s, kwargs=%s,XXXXXXXXXX.', cctxt, server, method, args, kwargs)
