...
class OpenStackClients(object):
    ...
    def __init__(self, context):
        #LOG.debug('OpenStackClients  xxx context=%s' % (context.to_dict()))
    ...
    def keystone(self):
        #LOG.debug('OpenStackClients  keystone xxx context=%s' % (self.context.to_dict()))
        if self._keystone:
            return self._keystone

        self._keystone = keystone.KeystoneClientV3(self.context)
        #LOG.debug('OpenStackClients  keystone2 xxx _keystone=%s' % (self._keystone.__dict__))
        return self._keystone
    ...
    @exception.wrap_keystone_exception
    def neutron(self):
        #LOG.debug('client neutron  xxx start')
        if self._neutron:
            return self._neutron

        session = self.keystone().session
        endpoint_type = self._get_client_option('neutron', 'endpoint_type')
        #LOG.debug('client neutron  xxx session=%s, session_dict=%s, endpoint_type=%s' % (session, session.__dict__, endpoint_type))
        #LOG.debug('client neutron  xxx _get_auth=%s, auth_token=%s' % (self.keystone()._get_auth(), self.keystone().auth_token))
