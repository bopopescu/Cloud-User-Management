...
class Manager(periodic_task.PeriodicTasks):
    ...
    def _do_carrier_create_base(self, context, container, requested_networks,
                                  limits=None, reraise=False):
        LOG.error("xxx _do_carrier_create_base")
        container.task_state = consts.CONTAINER_CREATING
        container.save(context)
        try:
            self._update_task_state(context, container, None)
    ...
    def _do_carrier_create(self, context, container, requested_networks,
                             limits=None, reraise=False):
        LOG.debug('Creating container: %s', container.uuid)

        created_container = self._do_carrier_create_base(context,
                                                           container,
                                                           requested_networks,
                                                           limits,
                                                           reraise)
    ...
    def carrier_create(self, context, limits, requested_networks, container):
        self._do_carrier_create(context, container, requested_networks,limits, False)
    ...
    @translate_exception
    def carrier_delete(self, context, container, force):
        LOG.debug('Deleting carrier: %s, context=%s, force=%s', container.uuid, context, force)
        self._update_task_state(context, container, consts.CONTAINER_DELETING)
        reraise = not force
    ...
    @translate_exception
    def carrier_show(self, context, container):
        LOG.debug('Showing carrier: %s', container.uuid)
        try:
            #container = self.driver.show(context, container)
            if container.obj_what_changed():
                container.save(context)
    ...
    @translate_exception
    def carrier_update(self, context, container, patch):
        LOG.debug('Updating a carrier: %s', container.uuid)
        # Update only the fields that have changed
        for field, patch_val in patch.items():
            if getattr(container, field) != patch_val:
                setattr(container, field, patch_val)
    ...
    @periodic_task.periodic_task(run_immediately=True)
    def delete_unused_carriers(self, context):
        """Delete container with status DELETED"""
        # NOTE(kiennt): Need to filter with both status (DELETED) and
        #               task_state (None). If task_state in
        #               [CONTAINER_DELETING, SANDBOX_DELETING] it may
        ...
        if containers:
            for container in containers:
                try:
                    msg = ('%(behavior)s deleting carrier '
                           '%(container_name)s with status DELETED')
                    LOG.info(msg, {'behavior': 'Start',
                                   'container_name': container.name})
                    self.carrier_delete(context, container, True)
                    LOG.info(msg, {'behavior': 'Complete',
                                   'container_name': container.name})
                except exception.DockerError:
                    return
