...
@profiler.trace("db")
def list_carriers(context, filters=None, limit=None, marker=None,
                    sort_key=None, sort_dir=None):
    """List matching carriers.

    Return a list of the specified columns for all carriers that match
    the specified filters.
    :param context: The security context
    :param filters: Filters to apply. Defaults to None.
    :param limit: Maximum number of carriers to return.
    :param marker: the last item of the previous page; we return the next
                   result set.
    :param sort_key: Attribute by which results should be sorted.
    :param sort_dir: Direction in which results should be sorted.
    ...
    :returns: A list of tuples of the specified columns.
    """
    return _get_dbdriver_instance().list_carriers(
        context, filters, limit, marker, sort_key, sort_dir)


@profiler.trace("db")
def create_carrier(context, values):
    """Create a new carrier.

    :param context: The security context
    :param values: A dict containing several items used to identify
                   and track the carrier, and several dicts which are
                   passed
                   into the Drivers when managing this carrier. For
                   example:
                   ::
                    {
                     'uuid': uuidutils.generate_uuid(),
    ...
    :returns: A carrier.
    """
    return _get_dbdriver_instance().create_carrier(context, values)


@profiler.trace("db")
def get_carrier_by_uuid(context, carrier_uuid):
    """Return a carrier.

    :param context: The security context
    :param carrier_uuid: The uuid of a carrier.
    :returns: A carrier.
    """
    return _get_dbdriver_instance().get_carrier_by_uuid(
        context, carrier_uuid)


@profiler.trace("db")
def get_carrier_by_name(context, carrier_name):
    """Return a carrier.

    :param context: The security context
    :param carrier_name: The name of a carrier.
    :returns: A carrier.
    """
    return _get_dbdriver_instance().get_carrier_by_name(
        context, carrier_name)


@profiler.trace("db")
def destroy_carrier(context, carrier_id):
    """Destroy a carrier and all associated interfaces.

    :param context: Request context
    :param carrier_id: The id or uuid of a carrier.
    """
    return _get_dbdriver_instance().destroy_carrier(context, carrier_id)


@profiler.trace("db")
def update_carrier(context, carrier_id, values):
    """Update properties of a carrier.
    ...
    :raises: ContainerNotFound
    """
    return _get_dbdriver_instance().update_carrier(
        context, carrier_id, values)
