...
@profiler.trace("db")
def list_providers(context, filters=None, limit=None, marker=None,
                    sort_key=None, sort_dir=None):
    """List matching providers.

    Return a list of the specified columns for all providers that match
    the specified filters.
    :param context: The security context
    :param filters: Filters to apply. Defaults to None.
    ...
    :returns: A list of tuples of the specified columns.
    """
    return _get_dbdriver_instance().list_providers(
        context, filters, limit, marker, sort_key, sort_dir)


@profiler.trace("db")
def create_provider(context, values):
    """Create a new provider.

    :param context: The security context
    :param values: A dict containing several items used to identify
                   and track the provider, and several dicts which are
                   passed
                   into the Drivers when managing this provider. For
                   example:
                   ::
                    {
                     'uuid': uuidutils.generate_uuid(),
    ...
    :returns: A provider.
    """
    return _get_dbdriver_instance().create_provider(context, values)


@profiler.trace("db")
def get_provider_by_uuid(context, provider_uuid):
    """Return a provider.

    :param context: The security context
    :param provider_uuid: The uuid of a provider.
    :returns: A provider.
    """
    return _get_dbdriver_instance().get_provider_by_uuid(
        context, provider_uuid)


@profiler.trace("db")
def get_provider_by_name(context, provider_name):
    """Return a provider.

    :param context: The security context
    :param provider_name: The name of a provider.
    :returns: A provider.
    """
    return _get_dbdriver_instance().get_provider_by_name(
        context, provider_name)


@profiler.trace("db")
def destroy_provider(context, provider_id):
    """Destroy a provider and all associated interfaces.

    :param context: Request context
    :param provider_id: The id or uuid of a provider.
    """
    return _get_dbdriver_instance().destroy_provider(context, provider_id)


@profiler.trace("db")
def update_provider(context, provider_id, values):
    """Update properties of a provider.
    ...
    :raises: ContainerNotFound
    """
    return _get_dbdriver_instance().update_provider(
