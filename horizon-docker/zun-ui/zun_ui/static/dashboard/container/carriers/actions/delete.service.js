...
(function() {
  ...
  /**
   * @ngDoc factory
   * @name horizon.dashboard.container.carriers.delete.service
   * @Description
  ...
  angular
    ...
    .factory('horizon.dashboard.container.carriers.delete.service', deleteService);
  ...
  deleteService.$inject = [
    ...
    'horizon.dashboard.container.carriers.events',
  ...
  function deleteService(
    ...
    var notAllowedMessage = gettext("You are not allowed to delete carriers: %s");
    ...
    function createResult(deleteModalResult) {
      ...
      if (result.result.failed.length === 0 && result.result.deleted.length > 0) {
        if (/\/container\/carriers(\/)?$/.test($location.path())) {
          $route.reload();
        } else {
          $location.path('/admin/container/carriers');
        }
      } else {
    ...
    function deleteEntity(id) {
      return zun.deleteCarrier(id, true);
    }
  }
})();
