...
(function() {
  ...
  /**
   * @ngdoc overview
   * @name horizon.dashboard.container.carriers.update.service
   * @description Service for the container update modal
   */
  angular
    .module('horizon.dashboard.container.carriers')
    .factory('horizon.dashboard.container.carriers.update.service', updateService);
  ...
  updateService.$inject = [
    ...
    'horizon.dashboard.container.carriers.validStates',
  ...
  function updateService(
    ...
    function perform(selected) {
      ...
      // load current data
      zun.getCarrier(selected.id).then(onLoad);
      function onLoad(response) {
        config.model.name = response.data.name
          ? response.data.name : "";
        config.model.carrier_access_key_id = response.data.carrier_access_key_id
          ? response.data.carrier_access_key_id : "";
        config.model.carrier_access_key = response.data.carrier_access_key
          ? response.data.carrier_access_key : "";
        config.model.carrier_name = response.data.carrier_name
          ? response.data.carrier_name : "";
      }
    ...
    function submit(context) {
      ...
      return zun.updateCarrier(id, context.model).then(success);
    }
    ...
    function cleanUpdateProperties(model) {
      ...
      for (var key in model) {
        if (model.hasOwnProperty(key) && model[key] === null || model[key] === "" ||
            (key !== "name" && key !== "carrier_access_key_id" && key !== "carrier_access_key")) {
          delete model[key];
        }
      }
      return model;
