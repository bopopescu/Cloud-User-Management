...
PROVIDER_CREATE_ATTRS = zun_client.providers.CREATION_ATTRIBUTES
...
def _cleanup_provider_params(attrs, check, **params):
    args = {}
    run = False
...
def providerregion_create(request, **kwargs):
    ...
    args, run = _cleanup_params(PROVIDERREGION_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).providerregions.create(**args)
    return response

def provider_create(request, **kwargs):
    LOG.debug('xxx start provider_create xxx %s' % kwargs)
    args, run = _cleanup_params(PROVIDER_CREATE_ATTRS, True, **kwargs)
    response = None
    response = zunclient(request).providers.create(**args)
    return response
...
def providerregion_update(request, id, **kwargs):
    ...
    args, run = _cleanup_providerregion_params(PROVIDERREGION_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
...
def provider_update(request, id, **kwargs):
    '''Update Container

    Get current Provider attributes and check updates.
    And update with "rename" for "name", then use "update" for
    "cpu" and "memory".
    '''

    # get current data
    container = zunclient(request).providers.get(id).to_dict()
    account_name = container["id"]
    args, run = _cleanup_provider_params(PROVIDER_CREATE_ATTRS, True, **kwargs)
    # remove same values from new params
    _delete_attributes_with_same_value(container, args)
    # do rename
    name = args.pop("id", None)
    #LOG.debug('xxxx zun-ui client provider_update name=%s, args=%s, len=%s, tmp_account=%s', name, args, len(args),tmp_account)
    if len(args):
        zunclient(request).providers.update(id, **args)

    # do update
    if name:
        zunclient(request).providers.rename(id, name)
        args["id"] = name
...
def provider_delete(request, id, force=False):
    # TODO(shu-mutou): force option should be provided by user.
    return zunclient(request).providers.delete(id, force=force)
...
def provider_list(request, limit=None, marker=None, sort_key=None,
                   sort_dir=None, detail=True):
    # TODO(shu-mutou): detail option should be added, if it is
    # implemented in Zun API
    return zunclient(request).providers.list(limit, marker, sort_key,
                                              sort_dir)
...
def provider_show(request, id):
    return zunclient(request).providers.get(id)
