...
@urls.register
class Carrier(generic.View):
    """API for retrieving a single container"""
    url_regex = r'zun/carriers/(?P<id>[^/]+)$'

    @rest_utils.ajax()
    def get(self, request, id):
        LOG.debug('restapi Carrier xxxxxx get=%s, id=%s xxx' % (request, id))
        """Get a specific container"""
        return change_to_id(client.carrier_show(request, id).to_dict())
    ...
    def delete(self, request, id):
        ...
        Returns HTTP 204 (no content) on successful deletion.
        """
        return client.carrier_delete(request, id, force=True)
    ...
    def patch(self, request, id):
        ...
        Returns the Container object on success.
        """
        args = client.carrier_update(request, id, **request.DATA)
        LOG.debug('restapi inside patch Carrier xxxxxx args=%s, xxx' % (args))
        return args
...
@urls.register
class Carriers(generic.View):
    """API for Zun Containers"""
    url_regex = r'zun/carriers/$'
    ...
    def get(self, request):
        ...
        item under this is a Container.
        """
        result = client.carrier_list(request)
        LOG.debug('restapi Get yyyzzxxxxxx result= %s xxxxxx' % (result))
        return {'items': [change_to_id(n.to_dict()) for n in result]}
    ...
    def delete(self, request):
        ...
        """
        for id in request.DATA:
            client.carrier_delete(request, id)
    ...
    def post(self, request):
        ...
        If 'run' attribute is set true, do 'run' instead 'create'
        """
        LOG.debug('carriers restapi POST xxxxxx %s xxxxx %s xxx' % (request, request.DATA))
        new_container = client.carrier_create(request, **request.DATA)
        LOG.debug('zunclient post "%s" and url"%s"' % (new_container.uuid, new_container.to_dict()))
        return rest_utils.CreatedResponse(
            '/api/zun/carrier/%s' % new_container.uuid,
            new_container.to_dict())
